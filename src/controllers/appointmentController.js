...
export async function getAppointmentsByProfessional(req, res) {
  try {
    const professionalId = req.userId;
    ...
    return res.status(200).json({
      mensagem: "Agendamentos do profissional encontrados.",
      agendamentos: appointments,
    });
  } catch (err) {
    console.error("Erro ao buscar agendamentos do profissional:", err);
    return res.status(500).json({
      mensagem: "Erro interno ao buscar agendamentos do profissional.",
      error: err.message,
    });
  }
}
