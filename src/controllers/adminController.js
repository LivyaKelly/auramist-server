...
export async function changeRoleToAdmin(req, res) {
  try {
    ...
    if (user.role === 'ADMIN') return res.status(400).json({ mensagem: "Usuário já é admin" });
...
export async function listAdmins(req, res) {
  try {
    const admins = await prisma.user.findMany({ where: { role: Prisma.Role.ADMIN } });
...
export async function changeRoleFromAdminToClient(req, res) {
  try {
    ...
    if (!admin) return res.status(404).json({ mensagem: 'Usuário não encontrado.' });
    if (admin.role !== Prisma.Role.ADMIN) return res.status(400).json({ mensagem: 'Usuário não é admin' });
